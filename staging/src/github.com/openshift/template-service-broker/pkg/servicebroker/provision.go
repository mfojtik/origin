...
import (
	...
	util2 "github.com/openshift/template-service-broker/pkg/util"
)
...
func (b *Broker) ensureSecret(u user.Info, namespace string, brokerTemplateInstance *templateapiv1.BrokerTemplateInstance, instanceID string, preq *api.ProvisionRequest, template *templateapiv1.Template, didWork *bool) (*kapiv1.Secret, *api.Response) {
	...
	if err := util2.Authorize(b.kc.AuthorizationV1().SubjectAccessReviews(), u, &authorizationv1.ResourceAttributes{
		Namespace: namespace,
		Verb:      "create",
...
func (b *Broker) ensureTemplateInstance(u user.Info, namespace string, brokerTemplateInstance *templateapiv1.BrokerTemplateInstance, instanceID string, template *templateapiv1.Template, secret *kapiv1.Secret, didWork *bool) (*templateapiv1.TemplateInstance, *api.Response) {
	...
	createdTemplateInstance, err := templateclient.TemplateInstances(namespace).Create(templateInstance)
	if err == nil {
		*didWork = true
	...
	if kerrors.IsAlreadyExists(err) {
		...
		existingTemplateInstance, err := templateclient.TemplateInstances(namespace).Get(templateInstance.Name, metav1.GetOptions{})
...
func (b *Broker) ensureBrokerTemplateInstanceUIDs(u user.Info, namespace string, brokerTemplateInstance *templateapiv1.BrokerTemplateInstance, secret *kapiv1.Secret, templateInstance *templateapiv1.TemplateInstance, didWork *bool) (*templateapiv1.BrokerTemplateInstance, *api.Response) {
	...
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		newBrokerTemplateInstance, err := templateclient.BrokerTemplateInstances().Update(brokerTemplateInstance)
		switch {
		case err == nil:
		...
		case kerrors.IsConflict(err):
			var getErr error
			brokerTemplateInstance, getErr = templateclient.BrokerTemplateInstances().Get(brokerTemplateInstance.Name, metav1.GetOptions{})
			if getErr != nil {
				err = getErr
			}
		}
...
func (b *Broker) ensureBrokerTemplateInstance(u user.Info, namespace, instanceID string, didWork *bool) (*templateapiv1.BrokerTemplateInstance, *api.Response) {
	...
	newBrokerTemplateInstance, err := templateclient.BrokerTemplateInstances().Create(brokerTemplateInstance)
	if err == nil {
		*didWork = true
	...
	if kerrors.IsAlreadyExists(err) {
		...
		existingBrokerTemplateInstance, err := templateclient.BrokerTemplateInstances().Get(brokerTemplateInstance.Name, metav1.GetOptions{})
		if err == nil &&
...
func (b *Broker) Provision(u user.Info, instanceID string, preq *api.ProvisionRequest) *api.Response {
	...
	template, err := lister.GetByUID(preq.ServiceID)
	...
	out:
		for namespace := range templateNamespaces {
			templates, err := lister.Templates(namespace).List(labels.Everything())
			if err != nil {
	...
	if _, ok := templateNamespaces[template.Namespace]; !ok {
		return api.BadRequest(kerrors.NewNotFound(templateapiv1.Resource("templates"), preq.ServiceID))
	}
