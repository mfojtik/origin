...
type fakeBroker api2.Response

func (b *fakeBroker) Catalog() *api2.Response {
	r := api2.Response(*b)
	return &r
}

func (b *fakeBroker) Provision(u user.Info, instanceID string, preq *api2.ProvisionRequest) *api2.Response {
	r := api2.Response(*b)
	return &r
}

func (b *fakeBroker) Deprovision(u user.Info, instanceID string) *api2.Response {
	r := api2.Response(*b)
	return &r
}

func (b *fakeBroker) Bind(u user.Info, instanceID string, bindingID string, breq *api2.BindRequest) *api2.Response {
	r := api2.Response(*b)
	return &r
}

func (b *fakeBroker) Unbind(u user.Info, instanceID string, bindingID string) *api2.Response {
	r := api2.Response(*b)
	return &r
}

func (b *fakeBroker) LastOperation(u user.Info, instanceID string, operation api2.Operation) *api2.Response {
	r := api2.Response(*b)
	return &r
}

var _ api2.Broker = &fakeBroker{}
...
func checkResponseWriter(t *testing.T, rw *fakeResponseWriter) {
	expectedHeaders := map[string]string{
		...
		api2.XBrokerAPIVersion:     api2.APIVersion,
	}
...
func TestRequiresXBrokerAPIVersionHeader(t *testing.T) {
	...
	fb := fakeBroker(*api2.NewResponse(http.StatusOK, map[string]interface{}{}, nil))
	Route(c, "", &fb)
...
func TestRequiresContentTypeHeader(t *testing.T) {
	...
	c.ServeHTTP(rw, &http.Request{
		...
		Header: http.Header{
			api2.XBrokerAPIVersion: []string{api2.APIVersion},
		},
...
func TestInternalServerError(t *testing.T) {
	c := restful.NewContainer()
	fb := fakeBroker(*api2.InternalServerError(errors.New("test error")))
	Route(c, "", &fb)
	...
	c.ServeHTTP(rw, &http.Request{
		Method: http.MethodGet,
		URL:    parseUrl(t, "/v2/catalog"),
		Header: http.Header{api2.XBrokerAPIVersion: []string{api2.APIVersion}},
	})
	checkResponseWriter(t, rw)
...
func TestBadRequestError(t *testing.T) {
	c := restful.NewContainer()
	fb := fakeBroker(*api2.BadRequest(errors.New("test error")))
	Route(c, "", &fb)
...
func TestProvision(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			body: &api2.ProvisionRequest{
				ServiceID: validUUID,
				PlanID:    validUUID,
				Context: api2.KubernetesContext{
					Platform:  api2.ContextPlatformKubernetes,
					Namespace: "test",
				},
			},
		...
		},
		{
			...
			req: http.Request{
				URL: parseUrl(t, "/v2/service_instances/"+validUUID+"?accepts_incomplete=true"),
				Header: http.Header{
					http.CanonicalHeaderKey(api2.XBrokerAPIOriginatingIdentity): []string{defaultOriginatingIdentityHeader},
				},
			},
			body: &api2.ProvisionRequest{
				ServiceID: validUUID,
	...
	for _, test := range tests {
		...
		test.req.Header.Set(api2.XBrokerAPIVersion, api2.APIVersion)
