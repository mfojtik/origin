...
func getResourceFieldRef(from *corev1.EnvVarSource, c *corev1.Container) (string, error) {
	...
	internalFrom := kapi.EnvVarSource{}
	internalContainer := &kapi.Container{}
	if err := legacyscheme.Scheme.Convert(from, &internalFrom, nil); err != nil {
		return "", err
	}
	if err := legacyscheme.Scheme.Convert(c, internalContainer, nil); err != nil {
		return "", err
	}
	return resource.ExtractContainerResourceValue(internalFrom.ResourceFieldRef, internalContainer)
}
