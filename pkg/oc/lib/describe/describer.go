...
func (d *BuildDescriber) Describe(namespace, name string, settings kprinters.DescriberSettings) (string, error) {
	...
	if pod, err := d.kubeClient.Core().Pods(namespace).Get(buildapi.GetBuildPodName(buildObj), metav1.GetOptions{}); err == nil {
	...
	return tabbedString(func(out *tabwriter.Writer) error {
		...
		formatString(out, "Build Pod", buildapi.GetBuildPodName(buildObj))
