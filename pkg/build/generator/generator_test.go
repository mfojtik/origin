...
func TestGenerateBuildFromConfig(t *testing.T) {
	...
	*/
	...
	legacyscheme.Scheme.Convert(build, internalBuild, nil)
	validateErrors := validation.ValidateBuild(internalBuild)
	if len(validateErrors) > 0 {
		t.Fatalf("Unexpected validation validateErrors %v", validateErrors)
	}
}
...
func mockDockerStrategyForNilImage() buildv1.BuildStrategy {
...
func mockBuild(source buildv1.BuildSource, strategy buildv1.BuildStrategy, output buildv1.BuildOutput) *buildv1.Build {
	return &buildv1.Build{
		ObjectMeta: metav1.ObjectMeta{
			Name: "test-build",
		},
