...
func (b BuildDefaults) applyPodDefaults(pod *corev1.Pod, isCustomBuild bool) {
	...
	for _, c := range allContainers {
		...
		for i, v := range b.Config.Env {
			...
			legacyscheme.Scheme.Convert(v, &externalEnv[i], nil)
		}
		if isCustomBuild {
...
func (b BuildDefaults) applyBuildDefaults(build *buildv1.Build) {
	...
	for _, envVar := range b.Config.Env {
		...
		externalEnv := corev1.EnvVar{}
		legacyscheme.Scheme.Convert(envVar, &externalEnv, nil)
	...
	for _, lbl := range b.Config.ImageLabels {
		...
		legacyscheme.Scheme.Convert(lbl, &externalLabel, nil)
		addDefaultLabel(externalLabel, &build.Spec.Output.ImageLabels)
	}
	...
	for name, value := range defaultResources.Limits {
		var externalLimitName corev1.ResourceName
		if err := legacyscheme.Scheme.Convert(name, &externalLimitName, nil); err != nil {
			glog.Errorf("unable to convert resource name: %v", err)
			continue
		}
		if _, ok := build.Spec.Resources.Limits[externalLimitName]; !ok {
			glog.V(5).Infof("Setting default resource limit %s for build %s/%s to %v", name, build.Namespace, build.Name, value)
			build.Spec.Resources.Limits[externalLimitName] = value
		}
	}
	...
	for name, value := range defaultResources.Requests {
		...
		if _, ok := build.Spec.Resources.Requests[externalLimitName]; !ok {
			glog.V(5).Infof("Setting default resource request %s for build %s/%s to %v", name, build.Namespace, build.Name, value)
			build.Spec.Resources.Requests[externalLimitName] = value
		}
	}
}
